...
3rdparty/riot: 3rdparty/riot-web
	tmpdir=$$(mktemp -d); (git clone 3rdparty/riot-web $${tmpdir}/riot-web && rm -rf 3rdparty/riot && ( cd $${tmpdir}/riot-web && pnpm install && (VERSION=$$(git describe --abbrev=0 --tags) NX_DAEMON=false pnpm --filter element-web build) && cp apps/web/config.sample.json apps/web/webapp/ && cp apps/web/element.io/develop/config.json apps/web/webapp/develop.config.json && git describe --abbrev=0 --tags | cut -c 2- > apps/web/webapp/version) && mv $${tmpdir}/riot-web/apps/web/webapp 3rdparty/riot) || (cd 3rdparty/riot-web && pnpm remove @nextcloud/browserslist-config && exit 1); rc=$$?; rm -rf $${tmpdir}; exit $${rc}
